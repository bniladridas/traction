...
#include "RaceCameraTestGameMode.h"
...
ARaceCameraTestGameMode::ARaceCameraTestGameMode()
{
	DefaultPawnClass = ARaceVehicle::StaticClass();
}

void ARaceCameraTestGameMode::BeginPlay()
{
	Super::BeginPlay();
	...
	if (Track && Manager)
	{
			...
			FRotator(0.0f, GridYaw, 0.0f), P))
		{
			...
			UE_LOG(LogTemp, Display, TEXT("RACECAM10E2E: AI spawned"));
		}
	}
	...
	GetWorldTimerManager().SetTimer(H, [this]()
	{
		FActorSpawnParameters P;
		P.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		GetWorld()->SpawnActor<ATask10Probe>(ATask10Probe::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, P);
		UE_LOG(LogTemp, Display, TEXT("RACECAM10E2E: probe spawned"));
	}, 1.0f, false);
}
