...
#include "RaceFullRaceTestGameMode.h"
...
ARaceFullRaceTestGameMode::ARaceFullRaceTestGameMode()
{
	DefaultPawnClass = ARaceVehicle::StaticClass();
}

void ARaceFullRaceTestGameMode::BeginPlay()
{
	Super::BeginPlay();
	...
	if (Manager)
	{
		Manager->RaceConfig.LapCount = Task16Limits::RaceLaps;
		UE_LOG(LogTemp, Display, TEXT("RACEFULL16E2E: lap count %d for this run only"), Manager->RaceConfig.LapCount);
	}
	...
	if (Track && Manager)
	{
		...
		for (int32 Slot = 1; Slot <= 5; ++Slot)
		{
				...
				FRotator(0.0f, Yaw, 0.0f), P))
			{
				...
				UE_LOG(LogTemp, Display, TEXT("RACEFULL16E2E: AI slot %d spawned"), Slot);
			}
		}
	}
	...
	GetWorldTimerManager().SetTimer(H, [this]()
	{
		FActorSpawnParameters P;
		P.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		GetWorld()->SpawnActor<ATask16Probe>(ATask16Probe::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, P);
		UE_LOG(LogTemp, Display, TEXT("RACEFULL16E2E: probe spawned"));
	}, 1.0f, false);
}
