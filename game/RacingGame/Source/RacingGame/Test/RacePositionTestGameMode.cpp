...
#include "RacePositionTestGameMode.h"
...
ARacePositionTestGameMode::ARacePositionTestGameMode()
{
	DefaultPawnClass = ARaceVehicle::StaticClass();
}

void ARacePositionTestGameMode::BeginPlay()
{
	Super::BeginPlay();
	...
	// Second participant on the grid, behind the start line.
	if (Track && Manager)
	{
		...
		FActorSpawnParameters P;
		P.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		if (ARaceVehicle* Second = GetWorld()->SpawnActor<ARaceVehicle>(ARaceVehicle::StaticClass(), GridPos,
			FRotator(0.0f, GridYaw, 0.0f), P))
		{
			Manager->RegisterParticipant(Second);
			UE_LOG(LogTemp, Display, TEXT("RACEPOS11E2E: second participant spawned"));
		}
	}
	...
	GetWorldTimerManager().SetTimer(H, [this]()
	{
		FActorSpawnParameters P;
		P.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		GetWorld()->SpawnActor<ATask11Probe>(ATask11Probe::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, P);
		UE_LOG(LogTemp, Display, TEXT("RACEPOS11E2E: probe spawned"));
	}, 1.0f, false);
}
